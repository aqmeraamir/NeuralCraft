...
def load_checkpoint(load_image_model=True, filepath="src/saved_models/checkpoint.pth.tar"):
    print("=> Loading checkpoint")
    checkpoint = torch.load(filepath)

    # load the model based on given param
    if load_image_model: image_model.load_state_dict(checkpoint['state_dict'])
    else: schem_model.load_state_dict(checkpoint['state_dict'])
...
3D minecraft No. Model Params: {num_params_schem} \n\n\
...
if TRAIN_3D:
    ...
    # set the device + configure the optimizer as adam
